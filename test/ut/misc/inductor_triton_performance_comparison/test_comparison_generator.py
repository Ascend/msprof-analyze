...
from misc.inductor_triton_performance_comparison.comparison_generator import ComparisonGenerator  # noqa: E402
...
class TestComparisonGenerator(unittest.TestCase):
        ...
        expected_result = pd.Series(
            {
                ComparisonGenerator.COL_TRITON_OP: 'triton_unk_fused_native_layer_norm_0',
                ComparisonGenerator.COL_TRITON_DURATION: 30.0,
                ComparisonGenerator.COL_ORIGINAL_DURATION: 'aclnnReduceSum_ReduceSumOpAiCore_ReduceSum:100.00\naclnnDivs_RealDivAiCore_RealDiv:50.00',
                ComparisonGenerator.COL_ORIGINAL_TOTAL_DURATION: 150.0,
            }
        )
    ...
    def test_generate_compare_result_success_when_profiling_data_exists(
        self, mock_glob, mock_export, mock_file_manager, mock_path_manager
    ):
    ...
    @patch('misc.inductor_triton_performance_comparison.comparison_generator.logger')
    @patch('misc.inductor_triton_performance_comparison.comparison_generator.glob')
    def test_generate_compare_result_should_return_when_df_is_empty_or_none(
        self, mock_glob, mock_logger, mock_file_manager, mock_path_manager
    ):
        db_path = (
            "/path/to/output/inductor_triton/msprof_4131596_20260122065512633_ascend_pt/"
            "ASCEND_PROFILER_OUTPUT/ascend_pytorch_profiler.db"
        )
        mock_glob.glob.return_value = [db_path]
        generator = ComparisonGenerator(self.mock_params)
        generator.generate_compare_result()
        mock_logger.error.assert_called_with("Invalid profiling data, the db path is %s", db_path)
