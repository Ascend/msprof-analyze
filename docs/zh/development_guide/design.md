...
通过集群同一通信域内的闲置时间识别临近任务的模式建模，定位性能瓶颈点，自动识别提升自动化效率。
...
4. 对于硬件侧瓶颈，关注前后邻近任务之间执行的所有设备侧任务。对于这期间的任务，综合考虑这些操作的延时和操作频率，计算其综合延时，推选出综合延时最大的操作作为性能瓶颈操作。
...
| 敏感数据保护 | 是否将密码明文存储在配置文件中                               | 明文密码不允许写入配置文件（命令行工具安装部署及使用时必须配置密码的场景除外）。 |   否   |      |
