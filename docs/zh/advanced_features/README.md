...
本章汇总 `msprof-analyze` 在集群场景下的进阶分析功能，涵盖集群多维信息汇总、拆解对比、通信瓶颈定位和下发问题分析等专题。此外，支持通过 `Recipe` 规则进行自定义分析，具体开发指导请参见[自定义分析能力开发指导](./custom_analysis_guide.md)。

## 2. 使用前准备

支持以下两种集群数据：

* Ascend PyTorch Profiler 采集的 DB 格式集群数据，参见《[Ascend PyTorch调优工具](https://gitcode.com/Ascend/pytorch/blob/master/docs/zh/developer_notes/ascend_pytorch_profiler_user_guide.md)》
* msMonitor 采集的集群轻量化 DB 数据，参见《[msMonitor](https://gitcode.com/Ascend/msmonitor/blob/master/docs/zh/quick_start/msmonitor_quick_start.md)》
...
> [!NOTE]
>
> 昇腾950PR&950DT系列产品的CCU场景下由于不支持采集通信矩阵和通信算子带宽数据，因此该工具communication_time_sum和communication_matrix_sum分析功能不具有参考意义。
