...
显示类似以下信息，且出现 `All profiling data parsed`，表示训练及性能数据解析成功。损失值、时间、进程号和目录名称等信息会因运行环境而异。
...
若脚本提示没有空闲 NPU，请结束其他 NPU 任务或在其完成后重试。
...
3. 若需要定位明细数据，打开 `log/mstt_advisor_{timestamp}.xlsx`，结合报告中的模块名称和算子或 API 信息进一步分析；
4. 根据报告建议修改训练脚本、数据加载方式或通信配置后，重新采集 Profiling 数据并再次运行 Advisor，验证优化效果。

更多分析维度、参数和报告解读方法，请参见《[专家建议](../user_guide/advisor_instruct.md)》。
